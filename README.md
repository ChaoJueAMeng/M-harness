...
模型提出行动后，必须经过权限策略和 `WorkspaceGuard`，才能读写工作区或执行命令。Agent 模式在真正改文件前会把工作区拍成独立 git ref；失败、取消或手动回滚可恢复。

提供三种用法：

- **Windows 桌面端**（WinUI 3）：选工作区、Ask / Agent、批准框、设置、回滚
...
- Agent 模式需要本机 Git，工作区必须是 git 仓库（checkpoint 依赖它）
...
Ask 模式可以在非 git 目录里只读提问。安装包用户不必再装 JDK 或 .NET SDK。
...
- 选择工作区，用 **Ask**（只读）或 **Agent**（可改文件）发送任务
...
不要做成 MSIX：Agent 要在用户任选的 git 仓库里读写文件、执行 Shell，MSIX 沙箱会拦住 Java 子进程。正确做法是 **unpackaged 自包含发布 + 捆绑 JRE + Inno Setup**。
...
向别人分发时给 `Setup.exe` 即可。装好后不必再装 JDK 或 .NET SDK。Agent 模式仍需要本机 Git，工作区必须是 git 仓库。
...
| `run_terminal` | 在仓库根执行一条命令；超时 120 秒会杀掉进程树 |

变更类工具是 `search_replace`、`write_file`、`run_terminal`。Shell 工作目录锁定在仓库根。
